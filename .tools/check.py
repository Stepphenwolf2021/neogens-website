...
# ปีกกาในบล็อก <style> ต้องสมดุล   เพิ่มด่านนี้เมื่อ 2026-08-23
# เกิดจริง สคริปต์ตัดกฎ CSS มาแปะด้วย regex แล้วได้ @media ที่ขาดปีกกาปิดหนึ่งตัว
# กฎทุกข้อที่ตามหลังจึงถูกกลืนเข้าไปอยู่ในเงื่อนไข max-width:640px เมนูย่อยเลยกางค้าง
# บนจอกว้าง ทั้งที่ทุกบรรทัดอยู่ในไฟล์ครบ เปิดโค้ดดูไม่มีทางเห็น ต้องนับเอา
for f in files:
    src,p=parsed[f]
    for st in re.findall(r'<style[^>]*>(.*?)</style>',src,re.S):
        o,c=st.count('{'),st.count('}')
        if o!=c: E(f,f'<style> ปีกกาไม่สมดุล เปิด {o} ปิด {c} — กฎที่ตามหลังจะถูกกลืน')
